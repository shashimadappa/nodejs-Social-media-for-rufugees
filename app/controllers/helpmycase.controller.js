...
exports.createFunds = async (req, res) => {
    const { authorId,title, fundsNeeded , discription, tags, location, applicationDeadline, createdAt, updatedAt } = req.body;
  ...
  //    const userId = req.id;
    try {
      ...
      const fundsData = new helpmycaseTbl({
        authorId, title, discription, tags, location,fundsNeeded, applicationDeadline, createdAt, updatedAt
      });
  ...
  exports.findAll = async (req, res) => {
    try {
      ...
      const posts = await helpmycaseTbl.find().sort({ createdAt: -1 }).skip(skip).limit(limit);
